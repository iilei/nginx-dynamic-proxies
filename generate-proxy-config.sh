...
proxies=$( awk 'BEGIN{for (i in ENVIRON) {print i}}' | grep -E "${PROXY_PATH_PREFIX}${DELIM}" )
...
for proxy in $proxies; do
  ...
  eval "export PROXY_PATH=\${$PROXY_PATH_NAME}"
  eval "export PROXY_ENDPOINT=\${$PROXY_ENDPOINT_NAME}"
